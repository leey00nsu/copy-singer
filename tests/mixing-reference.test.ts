...
import { SMART_REFERENCE_MID_VERSION, SMART_REFERENCE_VERSION } from "../lib/vocal-profile/contract";
...
test("profiles snapshot the ready smart synthesis reference", () => {
  assert.equal(selectMixingReference({ userId: "owner", smart, source, contractVersion: SMART_REFERENCE_MID_VERSION })?.id, "smart");
  assert.equal(selectMixingReference({ userId: "owner", smart, source, contractVersion: SMART_REFERENCE_VERSION })?.id, "smart");
});

test("mid-only profiles reject source fallback when smart reference is unavailable", () => {
  assert.equal(selectMixingReference({ userId: "owner", smart: null, source, contractVersion: SMART_REFERENCE_MID_VERSION }), null);
  assert.equal(selectMixingReference({
    userId: "owner",
...
test("legacy and smart-reference-v1 profiles keep source fallback", () => {
  ...
  assert.equal(selectMixingReference({ userId: "owner", smart: null, source, contractVersion: SMART_REFERENCE_VERSION })?.id, "source");
...
test("mixing rejects references not owned by the requester", () => {
  assert.equal(selectMixingReference({ userId: "other", smart, source, contractVersion: SMART_REFERENCE_MID_VERSION }), null);
});
