...
test("rejects a catalog with a missing rank", async () => {
  ...
  assert.throws(() => parseSongCatalogMarkdown(missingSecond, 99), /Catalog order must be continuous/);
});
...
test("retries transient analyzer failures before marking a song failed", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async () => {
    requests += 1;
    if (requests === 1) {
      return new Response(JSON.stringify({ reasonCode: "YT_DLP_FAILED", detail: "temporary extractor failure" }), {
        status: 422,
