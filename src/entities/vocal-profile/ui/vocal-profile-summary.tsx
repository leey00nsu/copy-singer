  ...
  return (
    <section aria-labelledby="vocal-profile-summary-title" className="py-8 sm:py-10">
      ...
      <div className="mt-3 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,.8fr)] lg:items-end">
        ...
        <ul className="grid gap-3 sm:grid-cols-3" aria-label="관찰된 보컬 특징">
          {presentation.traits.map((trait) => (
            <li className="border-l pl-3" key={trait.id}>
      ...
      <dl className="mt-8 grid border-t sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([label, value, detail, Icon]) => {
          return (
            <div
              className="border-b py-5 sm:px-5 sm:even:border-l xl:border-b-0 xl:border-l xl:first:border-l-0"
              key={label}
            >
