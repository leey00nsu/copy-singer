...
export async function deleteMixingJobForUser(userId: string, id: string) {
  const resultAssetId = await prisma.$transaction(async (transaction) => {
    ...
    if (!job) throw new MixingError("MIXING_NOT_FOUND", "믹싱 작업을 찾을 수 없습니다.", 404);
    if (!TERMINAL_STATUSES.includes(job.status as (typeof TERMINAL_STATUSES)[number])) {
      throw new MixingError("MIXING_ACTIVE", "진행 중인 믹싱 작업은 삭제할 수 없습니다.", 409);
    }
    if (job.resultAsset && (job.resultAsset.userId !== userId || job.resultAsset.kind !== "MIX_RESULT")) {
      throw new MixingError("MIXING_RESULT_INVALID", "믹싱 결과 파일의 소유권을 확인할 수 없습니다.", 500);
    }
    ...
    if (deleted.count !== 1) {
      throw new MixingError("MIXING_DELETE_CONFLICT", "믹싱 작업 상태가 변경되어 삭제하지 못했습니다.", 409);
    }
