...
export const ANALYSIS_ERROR_GUIDANCE: Record<string, { action: string; title: string }> = {
  TOO_SHORT: { title: "녹음이 너무 짧아요", action: "5초 이상 노래한 뒤 다시 시도해 주세요." },
  TOO_LONG: { title: "녹음이 너무 길어요", action: "파일을 다시 선택한 뒤 60초 자르기를 선택해 주세요." },
  TOO_SILENT: { title: "목소리가 너무 작아요", action: "마이크에 조금 가까이 다가가 반주 없이 더 크게 불러 주세요." },
  EXCESSIVE_CLIPPING: { title: "소리가 찌그러졌어요", action: "마이크에서 조금 멀어지거나 입력 음량을 낮춰 주세요." },
  LOW_VOICED_RATIO: {
    title: "노래 음정을 충분히 찾지 못했어요",
    action: "말소리보다 모음 ‘아’로 길게, 반주 없이 다시 불러 주세요.",
  },
  PAYLOAD_TOO_LARGE: { title: "파일이 너무 커요", action: "25MB 이하 파일을 사용해 주세요." },
  UNSUPPORTED_AUDIO: { title: "지원하지 않는 오디오예요", action: "WAV, MP3, M4A 또는 WebM 파일을 사용해 주세요." },
  INVALID_SEGMENTS: { title: "녹음 구간을 확인하지 못했어요", action: "새로 녹음한 뒤 다시 분석해 주세요." },
  ANALYZER_UNAVAILABLE: { title: "목소리를 분석할 수 없어요", action: "잠시 뒤 다시 시도해 주세요." },
  ANALYZER_NOT_CONFIGURED: { title: "현재 목소리 분석을 사용할 수 없어요", action: "잠시 뒤 다시 시도해 주세요." },
  ANALYSIS_ENQUEUE_FAILED: { title: "목소리 분석을 시작하지 못했어요", action: "잠시 뒤 다시 시도해 주세요." },
  INSUFFICIENT_ANALYSIS_TICKETS: {
    title: "분석 티켓이 없어요",
...
export function recorderIssueFromError(error: unknown): RecorderIssue {
  ...
  if (name === "NotAllowedError" || name === "SecurityError") {
    return {
      ...
      title: "마이크 권한이 차단됐어요",
      description: "브라우저의 사이트 설정에서 마이크를 허용하거나 아래에서 오디오 파일을 올려 주세요.",
    };
  }
  if (["NotFoundError", "DevicesNotFoundError", "NotReadableError", "TrackStartError", "AbortError"].includes(name)) {
    return {
      ...
      description: "다른 앱이 마이크를 사용 중인지 확인하고, 장치를 다시 연결하거나 오디오 파일을 올려 주세요.",
    };
  }
  if (message.includes("MEDIA_RECORDER_UNAVAILABLE")) {
    return {
      ...
      description: "최신 브라우저로 다시 열거나 WAV, MP3, M4A, WebM 파일을 올려 주세요.",
    };
  }
  return {
    kind: "unknown",
    title: "마이크를 시작하지 못했어요",
    description: "마이크 연결과 브라우저 권한을 확인한 뒤 다시 시도하거나 오디오 파일을 올려 주세요.",
  };
}
