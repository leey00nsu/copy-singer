...
import { VocalProfileResults } from "@/entities/vocal-profile";
...
export default async function VocalProfileDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-7xl px-5 py-16 sm:px-8">
      ...
      <div className="mt-7 flex flex-wrap items-start justify-between gap-5">
        <div>
          <div className="flex items-center gap-2 text-emerald-700">
            ...
            <span className="text-sm font-semibold">저장된 분석</span>
          </div>
      ...
      <Card className="mt-8">
        <CardHeader>
          <CardTitle className="text-lg">제출한 보컬</CardTitle>
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        {detail.latestRecommendationId ? (
          <Link className={buttonVariants()} href={`/recommendations/${detail.latestRecommendationId}`}>
            <Sparkles className="size-4" /> 최근 추천 결과 보기
          </Link>
        ) : null}
