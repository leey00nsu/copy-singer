...
import { parseSongCatalogMarkdown, validateSongProfileArtifact } from "../src/entities/recommendation/index.server";
...
const artifact = validateSongProfileArtifact(JSON.parse(await readFile(artifactPath, "utf8")) as unknown, entries);
const counts = artifact.songs.reduce((result, entry) => ({ ...result, [entry.status]: result[entry.status] + 1 }), {
  READY: 0,
  FAILED: 0,
  PENDING: 0,
});
