...
import { getAdminOverview, listAdminMixingJobs, listAdminUsers } from "@/_pages/admin/index.server";
      ...
      <section className="mt-8 grid gap-8 xl:grid-cols-2">
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{userResult.total}명</span>
          </div>
              ...
              <tbody className="divide-y">
                {userResult.users.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td>{user.ticketBalance}</td>
                    <td>{user._count.vocalProfiles}</td>
                    <td>{user._count.mixingJobs}</td>
                    <td className="text-xs text-muted-foreground">{user.createdAt.toLocaleDateString("ko-KR")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-semibold">믹싱 작업</h2>
            <span className="text-xs text-muted-foreground">{jobResult.total}건</span>
          </div>
          <div className="overflow-x-auto rounded-2xl border bg-background">
            <table className="w-full min-w-[700px] text-left text-sm">
              <thead className="border-b bg-muted/40 text-xs text-muted-foreground">
                <tr>
                  <th className="p-3">곡</th>
                  <th>사용자</th>
                  <th>상태</th>
                  <th>시도</th>
                  <th>생성</th>
                ...
                {jobResult.jobs.map((job) => (
                  <tr key={job.id}>
                    <td className="p-3">
                      ...
                      <p className="text-xs text-muted-foreground">{job.song.artist}</p>
                    </td>
                    <td className="text-xs">{job.user.email}</td>
                    <td>
                      <Badge variant={job.status === "FAILED" ? "destructive" : "secondary"}>
                        {job.status.toLowerCase()}
                      </Badge>
                      {job.errorCode ? <p className="mt-1 text-[10px] text-destructive">{job.errorCode}</p> : null}
                    </td>
                    <td>{job.attempts}</td>
                    <td className="text-xs text-muted-foreground">{job.createdAt.toLocaleString("ko-KR")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
