...
function enqueueError(error: unknown) {
  ...
  if (code === "INVALID_IDEMPOTENCY_KEY") {
    return Response.json({ reasonCode: code, detail: "A valid Idempotency-Key header is required.", retryable: false }, { status: 400 });
  }
  if (code === "UNSUPPORTED_AUDIO") {
    return Response.json({ reasonCode: code, detail: "Use a WAV, MP3, M4A, or WebM audio file.", retryable: false }, { status: 415 });
  }
  if (code === "PAYLOAD_TOO_LARGE") {
    return Response.json({ reasonCode: code, detail: "Audio must be 25 MB or smaller.", retryable: false }, { status: 413 });
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ reasonCode: "INVALID_UPLOAD", detail: "Expected one multipart audio upload.", retryable: false }, { status: 400 });
  }
