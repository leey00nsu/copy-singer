...
export function MixingHistoryList({ initial }: { initial: MixingHistoryPayload }) {
  ...
  if (history.jobs.length === 0) {
    return <div className="rounded-3xl border border-dashed p-12 text-center"><Music2 className="mx-auto size-8 text-muted-foreground" /><p className="mt-4 font-medium">아직 믹싱 내역이 없어요.</p><p className="mt-2 text-sm text-muted-foreground">추천 목록에서 듣고 싶은 곡의 AI 믹싱을 시작해보세요.</p></div>;
  }
  return (
          ...
          <article className="rounded-2xl border bg-background p-5" key={job.id}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={job.status === "succeeded" ? "default" : "secondary"}>
                    {active ? <LoaderCircle className="size-3 animate-spin" /> : job.status === "succeeded" ? <CheckCircle2 className="size-3" /> : <AlertTriangle className="size-3" />}
              ...
              <div className="text-right text-xs text-muted-foreground">
                <p className="flex items-center justify-end gap-1"><Ticket className="size-3" /> {job.ticketCost}개 사용</p>
                <p className="mt-1 flex items-center justify-end gap-1"><Clock3 className="size-3" /> {new Date(job.createdAt).toLocaleString("ko-KR")}</p>
              </div>
            </div>
            {job.audioUrl ? (
            ...
            {job.error ? <p className="mt-4 rounded-xl bg-destructive/8 p-3 text-sm text-destructive">{job.error.detail}</p> : null}
          </article>
        );
      })}
    </div>
