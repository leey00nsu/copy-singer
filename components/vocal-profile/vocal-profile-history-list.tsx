    ...
    return (
      <div className="rounded-3xl border border-dashed p-12 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          노래 한 소절을 분석하면 음역과 제출한 보컬을 여기에 보관합니다.
        </p>
        <Link className={`${buttonVariants()} mt-6`} href="/profile">
          첫 프로필 만들기
        </Link>
      </div>
    );
        ...
        <article className="rounded-2xl border bg-background p-5 shadow-sm" key={profile.id}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                전체 {midiToNoteName(profile.minMidi)} – {midiToNoteName(profile.maxMidi)}
              </p>
            </div>
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-xl bg-muted/45 p-3">
              <Gauge className="mx-auto size-4 text-emerald-600" />
              <p className="mt-2 text-xs text-muted-foreground">안정도</p>
              <p className="mt-1 font-semibold">{Math.round(profile.pitchStability * 100)}%</p>
            </div>
            <div className="rounded-xl bg-muted/45 p-3">
              <Sparkles className="mx-auto size-4 text-emerald-600" />
              <p className="mt-2 text-xs text-muted-foreground">추천</p>
              <p className="mt-1 font-semibold">{profile.recommendationCount}</p>
            </div>
            <div className="rounded-xl bg-muted/45 p-3">
              <Music2 className="mx-auto size-4 text-emerald-600" />
              <p className="mt-2 text-xs text-muted-foreground">믹싱</p>
              <p className="mt-1 font-semibold">{profile.mixingCount}</p>
            </div>
          </div>
          <Link
