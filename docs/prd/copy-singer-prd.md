...
Copy Singer는 사용자의 테스트 가창을 분석해 보컬 프로필을 만들고, 노래별 보컬 프로필과 비교하여 원키 적합도와 추천 노래방 키를 계산한다. 최종적으로 적합한 노래 3곡과 설명을 제시하고, 기존 SoulX-Singer SVC 기능으로 사용자 목소리 합성 데모를 제공한다.
...
- 100곡 카탈로그가 준비되면 상위 3곡과 사람이 이해할 수 있는 추천 이유를 제공한다.
- 추천 곡 일부를 사용자의 목소리로 변환해 결과를 미리 들려준다.
...
5. 상위 3곡과 추천 이유 UI/API
6. 기존 합성 데모와 추천 결과 연결
