...
- **PRD-FR-063**: 보컬 프로필 무제한 누적과 보컬 분석 무제한 사용을 분리해 관리한다. 일반 사용자의 `USER` 보컬 프로필 최대 보유 수는 서버 환경변수 `VOCAL_PROFILE_MAX_USER_PROFILES`(기본 3)를 따르며 삭제된 프로필과 실패한 분석은 슬롯을 차지하지 않는다. 기존 단일 티켓은 종류 기반 지갑으로 일반화해 `VOCAL_ANALYSIS` 분석 티켓과 `AI_MIXING` 믹싱 티켓을 별도 잔액으로 관리하고, 기존 `User.ticketBalance`와 기존 원장은 데이터 손실 없이 `AI_MIXING`으로 이전해야 한다. 가입 지급은 `SIGNUP_VOCAL_ANALYSIS_TICKET_GRANT`(기본 5)와 `SIGNUP_MIXING_TICKET_GRANT`(기본 1), 분석 비용은 `VOCAL_PROFILE_ANALYSIS_TICKET_COST`(기본 1), 믹싱 비용은 기존 `MIXING_TICKET_COST`를 서버 설정으로 관리한다. 새 보컬 분석은 슬롯과 분석 티켓을 서버에서 검증하고 정상 job 접수 시 분석 티켓을 한 번 차감하며 terminal failure에서 접수 당시 비용을 한 번 환불한다. 성공한 프로필을 삭제해도 분석 티켓은 복구하지 않는다. 분석 화면은 `보컬 프로필 used/limit`과 `분석 티켓 balance`를 분리해 표시하고 티켓 부족을 슬롯 만석보다 우선 안내해야 하며, 계정 메뉴·계정 내역·관리자 조정은 분석/믹싱 티켓 종류를 명시해야 한다. 클라이언트는 정책 환경변수를 직접 읽지 않고 서버 응답만 사용하며 기존 사용자당 동시 분석 1개 제한, 프로필 삭제·추천·믹싱 계약은 유지한다.
