...
- [DONE][PRD-FR-004] T-F010-modal-vocal-profile-analysis-08 durable queue 기반 백그라운드 보컬 프로필 분석
  ...
  - Acceptance:
    ...
    - worker가 PostgreSQL lease/attempt/idempotency 계약으로 job을 claim하고 기존 sync Modal analyzer를 호출해 source/reference와 VocalProfile persistence를 완료한다.
...
- 2026-08-08 구현 승인: 사용자 응답 `완료처리해줘`를 workflow 승인 옵션 `A`로 기록함.
