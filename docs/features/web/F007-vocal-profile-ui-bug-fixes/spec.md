...
- 좁은 곡 편향을 줄이는 대칭 테시투라 겹침 기반 `key-fit-v2`
- 원키 적합도를 우선하고 추천 키 적합도와 키 이동 부담을 함께 반영하는 전체 곡 선택 점수 순위
- 카탈로그 100곡 전체 목록과 항목별 추천 이유 표시
- 추천 실행 직후 자동 합성 제거 및 항목별 `AI 믹싱` 시작
- 선택형 추천 합성의 `auto_pitch_shift=true` 제품 preset
...
- [x] 브라우저에서 60초 초과 파일을 선택하면 “파일의 길이가 너무 길어요. 자동으로 자를까요?” 대화상자가 표시된다.
- [x] `아니오`를 누르면 파일 선택이 취소되고 분석 요청을 보내지 않는다.
- [x] `예`를 누르면 최초 유효 음성 시작점부터 최대 60초 구간만 분석된다.
- [x] 자동 자른 표준 WAV가 recording source로 저장되어 후속 추천 합성 reference도 같은 구간을 사용한다.
- [x] 60초 이하 파일과 브라우저 녹음의 기존 흐름은 바뀌지 않는다.

### US-5: 원키 적합도를 우선하는 노래 추천

**As a** 내 음역에 잘 맞는 노래를 찾는 사용자
**I want** 원키에서 잘 맞는 정도가 추천 순위에 우선 반영되기를 원한다
**So that** 큰 키 이동이 필요한 좁은 음역 곡이 반복 1위가 되는 결과를 피하고 추천 근거를 일관되게 이해할 수 있다

**Acceptance Criteria:**

- [x] 곡 테시투라 겹침은 사용자·곡 양쪽 구간 폭을 반영해 좁은 곡에 일방적으로 유리하지 않다.
- [x] 순위는 원키 65%, 추천 키 35%와 단계형 키 이동 감점을 적용한 선택 점수 순으로 결정된다.
- [x] 실제 저장된 서로 다른 사용자 profile fixture에서 `아크라포빅`이 일률적으로 1위가 되지 않는다.
- [x] 추천 키 계산과 원키·조정 후 점수는 계속 제공하며 새로운 scoring version으로 기존 실행과 구분된다.
- [x] 원키 점수와 추천 키 계산은 별도 지표로 제공되어 사용자가 순위 근거를 확인할 수 있다.

### US-6: 전체 추천 순위에서 선택형 AI 믹싱

**As a** 내 목소리에 맞는 곡을 폭넓게 탐색하는 사용자
**I want** 100곡 전체 순위를 비교하고 원하는 곡만 AI 믹싱하고 싶다
**So that** 상위 3곡에 제한되지 않고 불필요한 GPU 합성 비용 없이 관심 있는 결과만 들어볼 수 있다

**Acceptance Criteria:**

- [x] 추천 결과는 카탈로그 100곡 전체를 선택 점수 내림차순과 결정적 동점 규칙으로 표시한다.
- [x] 추천 실행을 생성하거나 목록을 조회하는 것만으로 합성 작업이 시작되지 않는다.
- [x] 각 합성 전 항목에 `AI 믹싱` 버튼이 있고 누른 항목 하나만 기존 상태 흐름으로 합성된다.
- [x] 진행 중·성공·실패 상태와 결과 재생·다운로드·재시도는 해당 목록 항목에서 확인할 수 있다.
- [x] 목록에서 시작한 합성 요청은 `auto_pitch_shift=true` 고정 제품 preset을 사용한다.
...
- `key-fit-v2`는 사용자와 shift된 곡 테시투라의 겹친 길이를 두 구간 폭의 합으로 정규화하는 Sørensen–Dice 계수를 사용한다.
- 동일 구간은 1, 겹침이 없는 구간은 0이며 좁은 곡이 넓은 사용자 구간 안에 포함됐다는 이유만으로 1이 되지 않는다.
- 테시투라·극단음 초과 부담, confidence와 정수 shift 탐색 계약은 유지한다.

### FR-11: 원키 중심 전체 순위 선택

- 선택 점수는 `원키 적합도 × 0.65 + 추천 키 적합도 × 0.35 - 키 이동 감점`으로 계산한다.
- 절대 shift 0~6의 감점은 각각 `0, 1, 3, 7, 12, 20, 30`점이다.
- 동점은 원키 점수, 추천 키 점수, 작은 절대 shift, 카탈로그 순서로 결정한다.
- 선택 점수는 recommendation metrics에 저장하고 기존 원키·추천 키 적합도와 별도로 직렬화한다.

### FR-12: 추천 점수와 합성 설정의 분리

- selection score의 원키 중심 가중치는 합성 preset과 독립적으로 유지한다.
- UI는 원키 적합도, 추천 키와 선택 점수를 추천 근거로 표시하고 합성 결과에는 실제 자동 피치 이동 설정을 별도로 안내한다.

### FR-13: 전체 카탈로그 순위

- 새 추천 실행은 분석 완료된 카탈로그 100곡 모두를 selection score 순으로 rank 1~100에 저장하고 반환한다.
- 기존 원키·조정 후 점수, 추천 키, 이유 코드와 scoring version 계약은 모든 항목에 유지한다.
- 과거 3곡 실행 조회는 오류 없이 유지하되 새 실행만 전체 목록 계약을 적용한다.

### FR-14: 항목별 선택형 합성

- 추천 결과 컴포넌트는 초기 `not_started` 항목을 진행 중으로 표현하거나 자동 POST하지 않는다.
- 사용자가 `AI 믹싱`을 누른 항목만 합성 endpoint를 호출하며, 준비·대기·처리 중에는 중복 요청을 막는다.
- 실패 항목은 명시적 재시도 버튼을 제공하고 성공 항목은 오디오 재생과 다운로드를 유지한다.

### FR-15: 자동 피치 이동 제품 preset

- 추천 목록에서 시작한 합성은 `auto_pitch_shift=true`, `pitch_shift=0`을 사용한다.
- 개발용 자유 입력 Workbench의 advanced setting은 변경하지 않는다.
- 추천 UI는 결과를 고정 원키 데모라고 표시하지 않고 자동 피치 이동을 사용하는 AI 믹싱으로 설명한다.
...
- PRD Refs: `PRD-US-001`, `PRD-US-002`, `PRD-US-003`, `PRD-US-004`, `PRD-US-005`, `PRD-US-006`, `PRD-US-007`, `PRD-US-008`, `PRD-US-009`, `PRD-FR-001`, `PRD-FR-002`, `PRD-FR-003`, `PRD-FR-004`, `PRD-FR-008`, `PRD-FR-009`, `PRD-FR-010`, `PRD-FR-011`, `PRD-FR-016`, `PRD-FR-017`, `PRD-FR-018`, `PRD-FR-021`, `PRD-FR-022`, `PRD-DATA-005`, `PRD-NFR-005`, `PRD-NFR-008`
