...
- [ ] 곡 테시투라 겹침은 사용자·곡 양쪽 구간 폭을 반영해 좁은 곡에 일방적으로 유리하지 않다.
- [ ] Top 3는 원키 65%, 추천 키 35%와 단계형 키 이동 감점을 적용한 선택 점수 순으로 결정된다.
- [ ] 실제 저장된 서로 다른 사용자 profile fixture에서 `아크라포빅`이 일률적으로 1위가 되지 않는다.
- [ ] 추천 키 계산과 원키·조정 후 점수는 계속 제공하며 새로운 scoring version으로 기존 실행과 구분된다.
- [ ] 합성 카드와 책임 안내는 결과가 추천 키 시연이 아닌 원키 음색 데모임을 명확히 표시한다.
