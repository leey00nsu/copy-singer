...
- 원키 적합도를 우선하고 추천 키 적합도와 키 이동 부담을 함께 반영하는 Top 3 선택 점수
- 추천 카드 합성 결과의 `원키 음색 데모` 표시
...
### US-5: 원키 데모와 일치하는 노래 추천

**As a** 추천 카드에서 원곡 피치의 목소리 데모를 듣는 사용자
**I want** 원키에서 잘 맞는 정도가 추천 순위에 우선 반영되기를 원한다
**So that** 큰 키 이동이 필요한 좁은 음역 곡이 반복 1위가 되는 결과를 피하고 데모와 추천 근거를 일관되게 이해할 수 있다

**Acceptance Criteria:**

- [x] 곡 테시투라 겹침은 사용자·곡 양쪽 구간 폭을 반영해 좁은 곡에 일방적으로 유리하지 않다.
- [x] Top 3는 원키 65%, 추천 키 35%와 단계형 키 이동 감점을 적용한 선택 점수 순으로 결정된다.
...
- [x] 합성 카드와 책임 안내는 결과가 추천 키 시연이 아닌 원키 음색 데모임을 명확히 표시한다.
...
- `auto_pitch_shift=false`, `pitch_shift=0` 합성 preset은 변경하지 않는다.
- 합성 결과 제목과 안내는 `원키 음색 데모`를 사용하고 추천 노래방 키가 실제 합성 피치에 적용되지 않음을 유지한다.
...
- PRD Refs: `PRD-US-001`, `PRD-US-002`, `PRD-US-003`, `PRD-US-004`, `PRD-US-005`, `PRD-US-006`, `PRD-US-008`, `PRD-US-009`, `PRD-FR-001`, `PRD-FR-002`, `PRD-FR-003`, `PRD-FR-004`, `PRD-FR-008`, `PRD-FR-009`, `PRD-FR-010`, `PRD-FR-011`, `PRD-FR-018`, `PRD-FR-021`, `PRD-FR-022`, `PRD-DATA-005`, `PRD-NFR-005`
