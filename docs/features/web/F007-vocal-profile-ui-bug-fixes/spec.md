...
- [ ] 추천 결과는 카탈로그 100곡 전체를 선택 점수 내림차순과 결정적 동점 규칙으로 표시한다.
- [ ] 추천 실행을 생성하거나 목록을 조회하는 것만으로 합성 작업이 시작되지 않는다.
- [ ] 각 합성 전 항목에 `AI 믹싱` 버튼이 있고 누른 항목 하나만 기존 상태 흐름으로 합성된다.
- [ ] 진행 중·성공·실패 상태와 결과 재생·다운로드·재시도는 해당 목록 항목에서 확인할 수 있다.
- [ ] 목록에서 시작한 합성 요청은 `auto_pitch_shift=true` 고정 제품 preset을 사용한다.
