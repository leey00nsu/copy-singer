...
- **Trace**:
  ...
  - **머지 후 확인**: Feature tip `fc27a47`를 `main`에 fast-forward 통합했다. post-merge `pnpm test`, `pnpm run lint`, `pnpm exec tsc --noEmit`가 모두 통과했고 managed worktree와 로컬 Feature 브랜치 cleanup도 완료됐다.
- **Evidence**:
  - **Commit**: `990e85e` (Feature 문서), `0c9bd26` (T-F017-01), `8e40cf1` (T-F017-02), `6c00c3c` (T-F017-03), `5db46e0` (T-F017-04)
  - **PR**: 로컬 워크플로우로 원격 PR 없음
  - **Local merge**: `LOCAL_MERGE_VERIFIED`, `LOCAL_CLEANUP_COMPLETE` — base/feature tip `fc27a47`
