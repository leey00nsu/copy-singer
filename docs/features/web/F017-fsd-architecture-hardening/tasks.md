# Tasks: fsd-architecture-hardening
...
- **문서 상태**: Approved
- **레포**: copy-singer-web
- **브랜치**: `feat/fsd-architecture-hardening`
- **대기 중 변경 요청**: -
...
- [DONE][NON-PRD] T-F017-01 README FSD 구조와 공개 API 규칙 정리
  - Date: 2026-08-09
  - Acceptance:
    - README의 Layout이 실제 `app`, `src/_app`, `src/_pages`, widgets, features, entities, shared 구조와 각 책임을 설명한다.
    - 공식 Next.js FSD 패턴인 `_app`·`_pages` 유지 이유, 의존 방향과 실행 환경별 공개 API 규칙이 문서화된다.
    - Steiger와 저장소 보완 검사의 역할 및 현재 prefix 한계가 과장 없이 기록된다.
  - Checklist:
    - [x] 오래된 `components`·`lib/*` Layout 항목을 현재 경로로 교체
    - [x] Next.js adapter와 FSD layer 흐름 및 slice/segment 경계 설명
    - [x] `index.ts`, `index.model.ts`, `index.server.ts` 사용 기준 설명
