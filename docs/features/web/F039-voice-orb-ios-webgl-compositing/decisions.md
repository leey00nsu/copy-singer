...
- **Trace**:
  ...
  - **DONE 전 확정 시점**: alpha 0일 때 premultiplied RGB도 0이 되는 fragment output과 renderer option을 source contract로 고정했다. 두 inverse falloff는 기존 threshold 순서를 low/high로 재배치하고 외곽 `edgeMask`는 유지했다.
  - **머지 후 확인**: 실제 결과/영향
- **Evidence**:
  - **Commit**: T01 task commit
  - **PR**: PR 링크
  - **Test/Log**: `pnpm exec tsx --test tests/voice-orb-shader.test.ts` PASS 5/5; targeted Biome PASS 2 files.
