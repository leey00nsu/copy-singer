...
- [ ] 완료된 선택 카드의 닫힌 `결과 듣기`는 흰색 icon/text의 primary action이며, 플레이어를 연 뒤 `결과 닫기`는 outline action으로 낮아진다.
...
- 완료된 선택 카드에서 재생 전 `결과 듣기`는 icon과 text가 primary foreground를 상속하는 primary button, 재생 후 `결과 닫기`와 `결과 저장`은 outline button으로 표시한다.
