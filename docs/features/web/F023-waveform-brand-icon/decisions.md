...
- **Options**: (A) 공개 전 placeholder 유지, (B) 확인되지 않은 이메일·주소를 임의 기재, (C) placeholder를 제거하고 실제 구현·운영 정책만 고지.
- **Decision**: (C)를 채택했다. 공통 draft 상태와 경고 배너를 제거하고 운영 형태를 무료·비상업적 개인 토이 프로젝트로 명시한다. 약관에는 원본 음원 미제공과 등록 음원의 판매·광고·재배포·별도 AI 학습 미사용을 기록한다. 개인정보 처리방침은 유지하되 현재 제공되는 개별 데이터 삭제 기능과 계정 전체 삭제·별도 문의 기능 미제공 상태를 사실대로 표시한다.
- **Rationale**: 존재하지 않는 연락처와 기능을 만들지 않으면서 사용자가 현재 데이터·음원 처리 범위를 이해할 수 있다. 생성 결과의 허용 범위는 사용자가 이번 변경에서 별도로 제한하지 않기로 했으므로 추가하지 않는다.
- **Trace**:
