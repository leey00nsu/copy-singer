# Tasks: admin-song-catalog-clarity
...
- **문서 상태**: Approved
- **레포**: copy-singer-web
- **브랜치**: `feat/admin-song-catalog-clarity`
- **대기 중 변경 요청**: -
...
- [DONE][PRD-FR-059] T-F035-admin-song-catalog-clarity-01 곡·버전 상태 presentation 모델 정리
  - Date: 2026-08-16
  - Acceptance:
    - 현재 공개·교체 준비·이전 버전과 공개 불가 이유가 raw enum 없이 결정적으로 파생된다.
    - 보컬과 반주가 함께 있는 원곡 파일 하나가 분석과 믹싱에 쓰이는 계약이 view/presentation에 반영된다.
  - Checklist:
    - [x] 현재 view model과 source 정렬 규칙을 감사한다.
    - [x] 상태·버전 역할·다음 행동을 계산하는 순수 presentation helper와 단위 테스트를 추가한다.
...
| --- | --- | --- |
| `pnpm exec tsx --test tests/admin-song-catalog-ui.test.tsx` | `2026-08-16` | PASS — 5/5 |
