...
- **Trace**:
  ...
  - **T-F018-22**: dense recommendation row를 명시적 선택 control로 만들고 desktop sticky panel/mobile bottom Sheet에서 추천 키·핵심 근거·티켓 비용과 AI 믹싱 CTA를 제공한다. Song Detail은 각 row와 선택 panel의 전체 근거 deep link로 유지한다.
  - **T-F018-23**: mixing POST 성공 job을 hook 결과로 보존하고 response ID의 `/library/mixes/[id]`로 즉시 이동한다. recommendation detail과 mixing history Query invalidation은 navigation과 병행하며 목록·Song Detail의 공통 hook 경로에 동일하게 적용한다.
- **Evidence**: T-F018-20 targeted Storybook 2 files/8 tests, mixing UI 8/8, recommendation ranking 10/10·presentation/UI 17/17. T-F018-21 voice scan 12/12, Analysis Status/Success Storybook 2 files/7 tests. T-F018-22 recommendation 27/27, Recommendation Results Storybook 9/9. T-F018-23 Query 24/24·streaming 1/1, recommendation 27/27, Mixing UI 8/8, Recommendation/Song Detail Storybook 14/14. 각 checkpoint `pnpm run check` 통과(error 0, 기존 Biome warning 59건, architecture 4/4).
