...
2. **Bento interaction**: Aceternity Bento Grid와 Glowing Effect source를 Copysinger token으로 조정하고 pointer hover와 keyboard focus에 같은 경계 강조를 제공한다.
...
- Artwork color mapping은 중앙음·음역 폭·안정도·유성음·RMS를 양자화한 voice signature와 profile ID를 함께 hash해 `Berry`, `Forest`, `Ocean Blue`, `Northern Sky`, `Aurora Violet` 중 하나를 균등하게 선택한다. 분석값은 선택된 family 안의 hue spread·saturation·lightness를 제한된 범위에서 조절하고 profile ID는 결정성과 focal position·angle·미세 편차를 담당한다. Component는 이 analogous multi-radial gradient 위에 사이트 기본 grain opacity 25%에 가까운 neutral fractal noise와 soft vignette를 합성한다. 별도 bitmap이나 canvas runtime은 추가하지 않는다.
- Shared `AudioWaveformPlayer`는 WaveSurfer의 unplayed/progress/cursor 색을 brand token에서 읽되 진행 구간만 violet→blue→pink gradient로 강조한다. Decode 중에는 waveform container와 동일한 72px slot에 aria-hidden grain-gradient skeleton과 screen-reader loading status를 겹치고, `isReady` 전환 시 CSS opacity·scaleY crossfade만 수행한다. Error fallback의 native audio, interaction, segment playback과 reserved layout은 유지한다.
- `globals.css`가 signal·soft·chart 역할별 개별 brand color stop을 소유한다. Canvas waveform과 React Bits Gradient Text는 signal stop을 읽고, Recharts의 음역 bar·음정 histogram·pitch trace는 chart stop으로 각 SVG 안의 linear gradient를 조립한다. 사용처가 없는 gradient shorthand token은 두지 않으며 Small status/icon/stepper와 black primary CTA에는 gradient를 확산하지 않는다.
