...
- `globals.css`가 `brand-violet`·`brand-blue`·`brand-pink`와 signal/soft gradient를 소유한다. Canvas waveform과 React Bits Gradient Text의 raw 색은 이 token을 읽고, Recharts의 음역 bar·음정 histogram·pitch trace는 각 SVG 안의 linear gradient definition을 사용한다. Small status/icon/stepper와 black primary CTA에는 gradient를 확산하지 않는다.
