...
3. **Voice Orb**: 분석 bento card와 실제 active `ProcessHero`에 공통 Orb를 사용한다. `hue=294`, `rotateOnHover=false`, `hoverIntensity=0`을 고정하고 viewport/visibility/reduced-motion에 따라 RAF를 정지한다.
   Profile recorder는 이를 감싼 `VoiceSignalCore`를 사용한다. idle/requesting/stopping은 processing보다 느리고 저채도인 동일 shader를 유지하며, recording은 MediaStream analyser의 RMS·peak를 45ms 간격으로 smoothing해 `--signal-level` CSS 변수만 갱신하고 scale·glow에 명확히 반영한다. Reduced-motion·WebGL 실패에서는 정적 poster로 대체하고, React state는 고주파 신호에 사용하지 않으며 unmount/recording 종료에서 RAF, source, analyser와 AudioContext를 정리한다.
