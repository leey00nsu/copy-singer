...
- Hero entry와 공통 `RevealContent`의 수동 CSS keyframe·IntersectionObserver state는 Motion primitive로 통합하되, Orb WebGL RAF·녹음 analyser RAF·단순 hover transition은 각 책임에 더 적합한 기존 구현을 유지한다.
...
- Vocal profile artwork는 제출 보컬의 중앙음·음역 폭·안정도·유성음 비율·RMS를 색상 family·색차·채도·밝기에 매핑하고 profile ID seed로 위치·각도·미세 편차를 더한다. 같은 프로필은 모든 화면에서 같은 artwork를 유지하고 서로 다른 보컬은 구분되되, 한 artwork 안에서는 인접 hue ramp만 사용해 과도한 다색 혼합을 피한다.
- Artwork 표면은 Aurora Gradient Generator의 `Northern Sky`, `Ocean Blue`, `Forest`, `Berry`처럼 한 계열 안에서 명도가 넓게 번지는 recursive/radial color field 위에 약 25% 수준의 neutral grain을 겹친 질감을 사용한다. `Purple Dream`·`Cyberpunk`처럼 원거리 hue가 한 표면에 동시에 섞이는 조합은 제외하고, grain은 작은 목록 썸네일에서도 인지되되 텍스트 대비나 프로필 식별 색을 흐리지 않는다.
