...
- [ ] 의존성 설치 후 Husky hook이 자동 준비되며 별도의 수동 hook 설치 절차가 필요하지 않다.
- [ ] pre-commit은 staged된 Biome 지원 파일만 검사하고, 오류가 있으면 커밋을 실패시킨다.
- [ ] pre-commit은 전체 테스트나 전체 빌드를 실행하지 않으며 staged 파일 범위로 제한된다.
- [ ] hook은 사용자 동의 없이 파일을 자동 수정하거나 staging 상태를 변경하지 않는다.
- [ ] 개발자는 별도 명령으로 포맷/안전한 자동 수정을 실행할 수 있다.
...
- [ ] CI가 고정된 pnpm lockfile로 의존성을 설치하고 Biome 전체 검사, ESLint, TypeScript 검사를 수행한다.
- [ ] 기존 테스트/빌드 검증을 CI 품질 게이트에 연결하되 비밀 값이나 외부 운영 서비스에 의존하지 않는 재현 가능한 구성을 사용한다.
- [ ] 하나의 검사라도 실패하면 CI job이 실패한다.
- [ ] 로컬에서 CI와 같은 핵심 정적 검사를 재현할 수 있는 package script가 제공된다.
