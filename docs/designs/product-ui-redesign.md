...
- Hero headline의 `내 목소리` phrase는 React Bits Gradient Text 기반 violet data accent→blue→restrained pink gradient를 1.5초로 움직이고, reduced-motion에서는 동일 gradient를 정적으로 유지한다.
