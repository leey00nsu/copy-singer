...
- 녹음 전에는 경과 시간 숫자를 노출하지 않는다. Recording 진입 시 WebGL Orb를 재생성하지 않고 grayscale→brand color를 전환하며, visual region의 높이와 Orb 위치가 이어진 뒤 waveform이 opacity 0→1로 등장해 하단 copy/control을 자연스럽게 민다.
