...
- Voice Notes의 4개 editorial card는 React Bits Grainient 기반의 단일 shared canvas를 배경으로 사용해 하나의 visual family로 묶는다. 카드마다 별도 WebGL context를 만들지 않으며 reduced-motion·WebGL 미지원 환경에서는 정적 violet·blue grain gradient를 유지한다.
