...
- Recommended key는 React Bits Count Up source pattern으로 `−2 → 1 → −1 → 0`의 정해진 예시 순환을 사용한다. 21개 세로 bar 각각에서 neutral 원본 높이와 변경분을 겹치고, 차감 구간은 원본 상단 내부 cyan segment, 증가 구간은 원본 위 violet cap으로 표시한다. Visible label은 하단 `원본에서 N키 낮춤/높임` 한 줄만 둔다. Metric band의 `5초+`, `60초`, `3단계`는 animation 없는 정적 사실로 유지한다. 화면 밖·background·reduced-motion에서는 key visualizer의 RAF와 timer를 정지하거나 최종 정적 값을 표시한다.
